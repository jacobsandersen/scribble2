...
impl Serialize for Mf2Value {
    ...
    {
        match &self {
            Mf2Value::String(str) => str.serialize(serializer),

            Mf2Value::Boolean(bool) => bool.serialize(serializer),

            Mf2Value::Number(num) => num.serialize(serializer),

            Mf2Value::Float(float) => float.serialize(serializer),

            Mf2Value::Embedded(obj) => obj.serialize(serializer),

            Mf2Value::Object(obj) => obj.serialize(serializer),
        }
    }
}
    ...
    pub fn from_form(form: HashMap<String, Vec<String>>) -> Mf2Object {
        ...
        for (key, value) in form {
            if key == "h" {
                ...
                properties.insert(
                    key,
                    value.into_iter().map(|v| Mf2Value::String(v)).collect(),
                );
            }
        }
...
impl TryFrom<MicropubPayload> for Mf2Object {
    ...
    fn try_from(value: MicropubPayload) -> Result<Self, Self::Error> {
        match value {
            MicropubPayload::Json(json) => serde_json::from_value::<Mf2Object>(json),

            MicropubPayload::Form(form) => Ok(Mf2Object::from_form(form)),
        }
    }
}
