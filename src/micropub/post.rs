    ...
    pub fn get_string(&self, key: &str) -> Option<String> {
        match self {
            ...
            MicropubPayload::Form(form) => {
                form.get(key).and_then(|v| v.first().map(|s| s.to_string()))
            }
        }
    }
}
...
pub async fn handle(
    ...
    let action = match &body.payload {
        ...
        MicropubPayload::Form(form) => form
            .get("action")
            .and_then(|v| v.first())
            .map(|s| s.as_str())
            .unwrap_or("create")
...
pub async fn handle_media(
    ...
    if content_type.essence_str() != "multipart/form-data" {
        return Err(invalid_request(
            "media endpoint only supports multipart/form-data content type",
        ));
    }

    let body = decode_multipart(
        &state,
        token,
        content_type,
        body,
    ...
    if body.files.is_empty() {
        return Err(invalid_request(
            "received media upload request with zero files (media endpoint accepts one file, in field named `file`)",
        ));
    }
...
async fn decode_json(
    ...
    let token = token
        .ok_or_else(|| unauthorized("Bearer token is required when sending JSON requests"))?
        .0;
...
async fn decode_form(
    ...
    let token = get_token(state, token, &mut map).await?;
    Ok(MicropubBody {
        payload: MicropubPayload::Form(map),
        files: Vec::new(),
        token,
    })
}
...
async fn decode_multipart(
    ...
    content_type: Mime,
    body: Body,
    field_name_filter: Option<String>,
) -> Result<MicropubBody, Response> {
...
async fn get_token(
    state: &Arc<AppState>,
    maybe_token: Option<Extension<TokenInfo>>,
    ...
    match (maybe_token, form_token) {
        ...
        (None, Some(token)) => Ok(auth::validate_token_or_reject(&state, &token).await?),
        (None, None) => Err(unauthorized(
            "access_token is required when not specified in the header",
        )),
    }
}
